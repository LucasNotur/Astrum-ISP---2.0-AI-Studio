...
describe('public-flags', () => {
  ...
  it('retorna false quando a env está ausente', () => {
    ...
    expect(getPublicFlags()).toEqual({ hub: false, toolreg: false, safety: false });
  });

  it('retorna true para "true"', () => {
    process.env.INTELLIGENCE_HUB_ENABLED = 'true';
    expect(getPublicFlags()).toEqual({ hub: true, toolreg: false, safety: false });
  });
  ...
  it('IA-19: TOOL_REGISTRY_ENABLED controla a chave toolreg', () => {
    ...
    expect(getPublicFlags()).toEqual({ hub: false, toolreg: true, safety: false });
  });
  ...
  it('IA-21: SAFETY_CLASSIFIER_ENABLED controla a chave safety', () => {
    ...
    expect(getPublicFlags()).toEqual({ hub: false, toolreg: false, safety: true });
  });
