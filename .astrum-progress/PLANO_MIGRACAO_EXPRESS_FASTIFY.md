...
**Próximos BUILD (Apêndice C, 🟢):** ✅ **TODOS os 🟢 feitos.** Resta: **REMOVER** `/api/ai/ask`, `/api/knowledge/articles*` (chamadas mortas) + **VERIFICAR** os T2/❓ (upsell, unmask, voip, service-orders/sync, backup, rag/scrape, incidents, tickets/human-response, billing) — ver Apêndice C.
