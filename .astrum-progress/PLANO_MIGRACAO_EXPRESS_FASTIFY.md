...
**Estado:** Fase 0 ✅. **Fase 1 ✅ essencialmente concluída** — todos os 🟢 construídos; REMOVER/VERIFICAR triados (4 dos 9 resolvidos: rag/scrape-url, tickets/human-response [migration 099], backup removido, incidents adaptado). Os 5 🟡 restantes viraram **backlog de produto** (billing, upsell, unmask, voip, service-orders/sync — cada um precisa de DECISÃO, não é roteamento; detalhes no Apêndice C).
**Feito:** `webhooks` (T1 repoint) + BUILDs completos: **LGPD-expunge**, **integrations (ERP)**, **departments** (migration 097), **metrics** fcr+time+fcr-target (migration 098), **whatsapp/health-stats** (sem migration — Redis + fila), **personas + keys + prompts/validate** (sem migration — legacy_docs/helper de db), **quality/live-stats** (sem migration — agregados de tickets), **settings/holidays/fetch-national** (sem migration — feriados nacionais computados localmente), **domains/verify** (DNS via node:dns), **knowledge/articles** (repoint Supabase), **rag/scrape-url** (SSRF-guarded), **tickets/human-response** (migration 099), **backup** (removido), **incidents** (adaptado p/ /api/v2/rede/incidents). Migrations 097/098/099 aplicadas via MCP + registradas em `schema_migrations`.
...
**Próximo (ordem combinada):** (1) **BACKLOG 🟡** — 5 features meia-construídas, cada uma precisa de decisão do dono antes de construir: **billing** (achar a fonte SaaS do provedor), **upsell** (unificar cobrai_jobs+manual + repoint dashboard), **unmask** (definir contrato de reveal de PII + auditoria), **voip** (design do bridge click-to-call + SDK/creds twilio), **service-orders/sync** (construir a metade frontend que enfileira, ou remover scaffolding). Também pendentes: `/api/ai/ask` (Fase 2, cutover S74) e reindex/search-test (gap vector/RAG). (2) Depois **FASE 2** (portar rotas Express legadas). Ver Apêndice C p/ o detalhe de cada.
...
**Triagem investigada 2026-08-13/14** (call site do front + backend disponível). Rec: 🟢 caminho claro · 🟡 precisa decisão.
**CONCLUSÃO (2026-08-14):** dos 9, **4 já resolvidos** (rag/scrape-url ✅, tickets/human-response ✅, backup ✅ removido, incidents ✅ adaptado). Os **5 restantes NÃO são migração mecânica — são features meia-construídas** (verificado item a item): **billing** (sem casa no Supabase), **upsell** (2 fontes + repoint do dashboard), **unmask** (contrato de segurança indefinido), **voip** (subespecificado, sem telefone do operador), **service-orders/sync** (fila nunca populada). Viraram **backlog de PRODUTO** (decidir/terminar a feature), não roteamento. → **Fase 1 essencialmente concluída;** próximo passo de MIGRAÇÃO é a **Fase 2** (portar rotas Express legadas).
...
| `/api/unmask` (MaskedSensitiveData) | ✅ **CONSTRUÍDO (2026-08-14, no Claude).** Design descoberto: a máscara é COSMÉTICA/client-side (o browser já tem o valor cru) → o endpoint não protege dado, o valor é a **auditoria LGPD**. `POST /api/v2/security/unmask {value,type,reason}` (`unmask.service.ts` puro teste 6/6 + `unmask.routes.ts`). Auditoria **privacy-preserving**: NÃO guarda PII cru, só type+reason+value_hash(sha256)+hint. Grava em `audit_log` (imutável, 095) via service_role, **fail-closed**. Front repontado via `apiPost`. | ✅ feito | 🟢 |
| `/api/voip/initiate-call` (Chat) | 🎯 **ARQUITETURA DECIDIDA + DB PRONTO (2026-08-14).** Decisão do dono: **WebRTC softphone via SIP.js + trunk SIP de operadora BR** (número +55 local crítico p/ atendimento; custo BR menor). DB feito: tabela `voip_calls` (CDR, migration 101, RLS). **Bloqueado** até haver trunk SIP real (credenciais) — o Claude implementa aqui (é sensível: senha SIP não vai pro browser). NÃO é tarefa do outro modelo. Ver `HANDOFF_BACKLOG_SPECS.md` SPEC 3. | ⏳ aguarda trunk SIP | 🟡 |
| `/api/upsell/convert` (App) | 🔨 **DB FEITO + HANDOFF (2026-08-14).** DashboardPage lê upsells de `cobrai_jobs` (semântica errada). Criada a tabela `upsell_events` (migration 100, RLS tenant_own, aplicada+registrada). **Código puro** (rota `POST /api/v2/upsell/convert` + repoint App.tsx + repoint leitura do DashboardPage) → spec detalhado em `HANDOFF_BACKLOG_SPECS.md` p/ o outro modelo executar; auditoria depois no Claude. | ⏳ código no handoff | 🟢 |
| `/api/service-orders/sync` (TechnicianApp) | ✅ **SCAFFOLDING REMOVIDO (2026-08-14).** A fila IDB `sync-queue`+`syncPending` nunca eram populadas (nada enfileirava) → código morto. Removidos (store, syncPending, listeners online/SW, import toast órfão). Offline-sync fica p/ quando for prioridade real (exigiria construir a metade frontend de enfileiramento). | ✅ feito | 🟡 |
...
| `/api/billing/subscription\|invoices/:tenantId` (Billing, T2) | ✅ **DECIDIDO + EXECUTÁVEL NO HANDOFF (2026-08-14).** Descoberta: a assinatura do provedor **É real e computável** — Escada Astrum (R$2,50 × `subscriber_count`, `plans.ts`) + campos reais em `tenants` (plan/active/trial_ends_at/subscriber_count). Decisão do dono ("A+B"): mostrar assinatura **verdadeira** (client-side via `plans.ts`, sem backend) + faturas em **empty-state honesto** (não há geração de fatura SaaS). NÃO usar `billing_plans` (é catálogo do ISP). Código puro (só BillingPage) → SPEC 2 executável no `HANDOFF_BACKLOG_SPECS.md`. | ⏳ código no handoff | 🟢 |
