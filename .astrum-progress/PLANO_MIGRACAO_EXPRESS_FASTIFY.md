...
| `/api/service-orders/sync` (TechnicianApp) | ⚠️ **INERTE (verificado 2026-08-14).** A fila IDB `sync-queue` é criada (linha 21), drenada (111) e deletada (121), mas **NADA escreve nela** em lugar nenhum do app → `queue` sempre vazio, `syncPending` sempre retorna cedo, o endpoint nunca é chamado. Construir o backend não serve pra nada sem a metade frontend (enfileirar mutações de OS offline). | ⏸️ adiar/remover scaffolding | 🟡 |
