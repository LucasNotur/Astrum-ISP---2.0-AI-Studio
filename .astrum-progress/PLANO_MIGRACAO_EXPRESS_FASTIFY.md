...
| `/api/upsell/convert` (App) | ✅ **CONCLUÍDO (2026-08-15).** DB pelo Claude (`upsell_events`, migration 100). Código puro pelo modelo externo (GLM/DeepSeek): rota `POST /api/v2/upsell/convert` (service puro `sanitizeUpsellInput` teste 7/7, tenant+operador do JWT) + App.tsx via `apiPost` + DashboardPage lê `upsell_events`. **Auditado pelo Claude** (commit `b3efb02`): escopo limpo, isolamento OK, 0 erros novos de tipo. | ✅ feito+auditado | 🟢 |
...
| `/api/billing/subscription\|invoices/:tenantId` (Billing, T2) | ✅ **CONCLUÍDO (2026-08-15).** Assinatura do provedor computada client-side de verdade (Escada Astrum R$2,50 × `subscriber_count` via `plans.ts` + campos reais de `tenants`) + faturas empty-state honesto; 2 fetch 404 removidos. Código pelo modelo externo, **auditado pelo Claude** (commit `167586b`): usa `monthlyPriceCents` certo, não toca `billing_plans`, legacy typecheck limpo. | ✅ feito+auditado | 🟢 |
