...
| `/api/voip/initiate-call` (Chat) | ⚠️ **SUBESPECIFICADO (verificado 2026-08-14).** Telefonia do apps/api é só INBOUND; **sem SDK twilio** no package.json, **sem `calls.create`**, env `TWILIO_*` opcionais. Pior: o payload do front tem `toNumber` (cliente) mas **não o telefone do operador** → não dá pra montar o bridge click-to-call. Precisa design do fluxo (operador→cliente) antes de construir. | ⏸️ design pendente | 🟡 |
